...
export default function Professeurs() {
  ...
  const [form, setForm] = useState({
    nom: '', prenom: '', email: '', mot_de_passe: '', telephone: '', specialite: '',
    adresse: '', npa: '', lieu: '', sexe: '', taux_activite: '', avs: '', type_contrat: '', type_permis: ''
  });
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={styles.formGrid}>
                ...
                <div style={styles.formChamp}>
                  ...
                  <input style={styles.input} type="number" min="10" max="100" step="5" value={form.taux_activite} onChange={e => setForm({ ...form, taux_activite: e.target.value })} placeholder="Ex: 100" />
                </div>
                <div style={styles.formChamp}>
      ...
      <table style={styles.table}>
        <thead>
          <tr style={styles.theadRow}>
            {['Nom', 'Prénom', 'Email', 'Spécialité', 'Contrat', 'Taux', 'Statut', 'Date création'].map(h => (
        ...
        <tbody>
          {profsFiltres.length === 0 ? (
            <tr><td colSpan="9" style={styles.vide}>Aucun professeur trouvé</td></tr>
          ) : profsFiltres.map(p => (
            <tr key={p.id} style={styles.tr}>
              <td style={styles.td}><b>{p.nom}</b></td>
              <td style={styles.td}>{p.prenom}</td>
              <td style={styles.td}>{p.email}</td>
              <td style={styles.td}>{p.specialite || '—'}</td>
              <td style={styles.td}>{p.type_contrat ? <span style={styles.badgeContrat}>{p.type_contrat}</span> : '—'}</td>
              <td style={styles.td}>{p.taux_activite ? p.taux_activite + '%' : '—'}</td>
              <td style={styles.td}><span style={styles.statutBadge}>Actif</span></td>
...
const styles = {
  ...
  statutBadge: { background: '#e8f5e9', color: '#2e7d32', padding: '3px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: '600' },
  badgeContrat: { background: '#e3f2fd', color: '#1a73e8', padding: '3px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: '600' },
