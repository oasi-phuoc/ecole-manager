...
export default function Notes() {
  ...
  const handleImprimer = (opts = {}) => {
    ...
    if (vue === 'bulletin') {
      ...
      const tdNoDiv = `${tdC}border-right:none;`;
      const tdTopNoDiv = `${tdNoDiv}border-top:${blockBorder};`;
      const tdBottomNoDiv = `${tdNoDiv}border-bottom:${blockBorder};`;
      ...
      const pages = elevesToShow.map(eleve => {
        ...
        const brRow=(n)=>`<tr><td style="${td}">${n}</td><td style="${tdC}">${pm1[n]?.moyenne!=null?fv(pm1[n].moyenne):'—'}</td><td style="${tdCR}">${showS2&&pm2[n]?.moyenne!=null?fv(pm2[n].moyenne):'—'}</td></tr>`;
    ...
    return (
      <div style={s.page}>
        ...
        {bulletinPopupEleve && (() => {
          ...
          const tdNoDiv = { ...tdC, borderRight: 'none' };
          const tdTopNoDiv = { ...tdNoDiv, borderTop: '1px solid #cbd5e1' };
          const tdBottomNoDiv = { ...tdNoDiv, borderBottom: '1px solid #cbd5e1' };
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 10, width: '100%' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 0 }}>
                        ...
                        <table style={{ ...s.tbl, ...tblBorder, width: '100%', tableLayout: 'fixed' }}>
                          ...
                          <tbody>
                            {secondaires.length === 0 && <tr><td colSpan={3} style={{ ...tdL, borderRight: '1px solid #cbd5e1', color: '#aaa' }}>—</td></tr>}
                            ...
                            <tr style={{ ...s.tr, fontWeight: 700 }}><td style={tdTopL}>Moyenne semestrielle</td><td style={tdTopNoDiv}>{moyG1 != null ? fmtNote(moyG1) : '—'}</td><td style={tdTopCR}>{showS2popup && moyG2 != null ? fmtNote(moyG2) : '—'}</td></tr>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                            ...
                            {BULLETIN_CRITERES_LABELS.map((label, idx) => {
                              ...
                              return (
                                <tr key={idx} style={{ ...s.tr, height: '1px' }}>
                                  ...
                                  <td style={isLast ? tdBottomNoDiv : tdNoDiv}>{dot(cr1['c' + (idx + 1)])}</td>
                        ...
                        <table style={{ ...s.tbl, ...tblBorder, width: '100%', maxWidth: '100%', tableLayout: 'fixed' }}>
                          ...
                          <tbody>
                            ...
                            <tr style={s.tr}><td style={tdBottomL}>Absences non excusées</td><td style={tdBottomNoDiv}>{stS1?.absents ?? 0}</td><td style={tdBottomCR}>{stS2?.absents ?? 0}</td></tr>
                          </tbody>
                        </table>
                      </div>
                    </div>
                ...
                <div id={printId} style={{ border: '2px solid #dc2626', borderRadius: 0, padding: '16px 24px 12px', fontFamily: font, position: 'relative', background: 'white', height: '194mm', maxHeight: '194mm', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 22, fontWeight: 800, color: '#1e293b' }}>SCAI</div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#374151' }}>{as}</div>
