-- École Manager — schéma initial (état consolidé issu de initDB + tables lazy)
-- Phase 1 migration Supabase : Postgres compatible, Auth custom JWT conservée.
-- Appliqué via: supabase db push | psql "$DATABASE_URL" -f ...

-- ─── Utilisateurs & structure scolaire ───────────────────────────────────────
...
CREATE TABLE utilisateurs (
  ...
  role VARCHAR(20) CHECK (role IN ('admin','prof','eleve','parent')) NOT NULL,
...
CREATE UNIQUE INDEX utilisateurs_identifiant_uq
...
-- ─── Documents (base64 TEXT — Phase 2 : migrer vers Supabase Storage) ────────

CREATE TABLE documents_profs (
  id SERIAL PRIMARY KEY,
...
CREATE TABLE inventaire_branches (
  id SERIAL PRIMARY KEY,
...
-- Cours normal + soutien autorisés sur le même créneau (index partiels)
...
CREATE TABLE parametres_ecole (
  ...
  responsables_niveaux JSONB DEFAULT '[]'::jsonb,
...
CREATE TABLE sorties_scolaires (
  id SERIAL PRIMARY KEY,
...
CREATE TABLE bulletin_criteres (
  id SERIAL PRIMARY KEY,
  classe_id INTEGER REFERENCES classes(id) ON DELETE CASCADE,
  eleve_id INTEGER REFERENCES eleves(id) ON DELETE CASCADE,
...
CREATE TABLE niveaux (
  ...
  ordre INTEGER DEFAULT 0,
  periodes_normales INTEGER DEFAULT 20,
  periodes_soutien INTEGER DEFAULT 0
);
...
CREATE TABLE classes_enclassement (
  id SERIAL PRIMARY KEY,
...
CREATE TABLE affectations_eleves_enc (
  id SERIAL PRIMARY KEY,
...
-- ─── Calendrier / absences / devoirs / factures / visites / sondages ─────────
...
CREATE TABLE factures_validations (
  id SERIAL PRIMARY KEY,
  eleve_id INTEGER REFERENCES eleves(id) ON DELETE CASCADE,
...
CREATE TABLE factures_references (
  id SERIAL PRIMARY KEY,
  eleve_id INTEGER REFERENCES eleves(id) ON DELETE CASCADE,
  annee_scolaire VARCHAR(20) NOT NULL,
...
CREATE INDEX idx_sondage_questions_sondage ON sondage_questions(sondage_id);
...
CREATE INDEX idx_sondage_reponses_sondage ON sondage_reponses(sondage_id);

-- Tables créées lazy hors initDB
CREATE TABLE notes_personnelles (
  id SERIAL PRIMARY KEY,
...
CREATE TABLE calendrier_prof (
  id SERIAL PRIMARY KEY,
  prof_id INTEGER NOT NULL REFERENCES utilisateurs(id) ON DELETE CASCADE,
  date DATE NOT NULL,
  titre VARCHAR(200) NOT NULL,
  type VARCHAR(50) DEFAULT 'Autre',
