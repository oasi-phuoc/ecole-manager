...
const loginMfa = async (req, res) => {
  ...
  try {
    let userId = null;
    if (String(mfa_token).startsWith('legacy:')) {
      userId = parseInt(String(mfa_token).slice(7), 10);
    } else {
    ...
    const result = await pool.query(
      'SELECT id, nom, prenom, email, role, mfa_enabled, mfa_secret, mfa_backup_codes, auth_user_id, doit_changer_mdp FROM utilisateurs WHERE id=$1 AND actif = true',
      [userId]
    );
    const user = result.rows[0];
    ...
    writeAuthCookie(res, userPayload(user));
    let session = null;
    if (user.auth_user_id && user.email && process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      try {
        const { createClient } = require('@supabase/supabase-js');
        const admin = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, {
          auth: { persistSession: false, autoRefreshToken: false },
        });
        const { data } = await admin.auth.admin.generateLink({ type: 'magiclink', email: user.email });
        if (data?.properties?.hashed_token) {
          const { data: sessData } = await admin.auth.verifyOtp({
...
const changerMdp = async (req, res) => {
  ...
  try {
    ...
    const u = await pool.query('SELECT auth_user_id, email FROM utilisateurs WHERE id=$1', [req.user.id]);
    if (u.rows[0]?.auth_user_id) {
      try {
        const { createClient } = require('@supabase/supabase-js');
        const admin = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY, {
          auth: { persistSession: false, autoRefreshToken: false },
        });
        await admin.auth.admin.updateUserById(u.rows[0].auth_user_id, { password: nouveau_mdp });
      } catch (_) {}
    }
