...
export async function handleNotesRoute(
  ...
  try {
    if (path === "/notes/classes-responsables" && req.method === "GET") {
    ...
    if (path === "/notes/semestre-config" && req.method === "GET") {
      try {
        await pool.query("CREATE TABLE IF NOT EXISTS app_settings (cle TEXT PRIMARY KEY, valeur TEXT)");
        const result = await pool.query(
          "SELECT valeur FROM app_settings WHERE cle = 'sem1_bloque' LIMIT 1",
        );
        return json(cors, {
          sem1_bloque: result.rows.length > 0 && result.rows[0].valeur === "true",
        });
      } catch {
        return json(cors, { sem1_bloque: false });
      }
    }

    if (path === "/notes/semestre-config" && req.method === "PUT") {
      const denied = requireAdmin(user!, cors);
      if (denied) return denied;
      const body = await parseJsonBody(req);
      const { sem1_bloque } = body;
      await pool.query("CREATE TABLE IF NOT EXISTS app_settings (cle TEXT PRIMARY KEY, valeur TEXT)");
      await pool.query(
      ...
      return json(cors, { message: "OK" });
    }

    if (path === "/notes/bulletin" && req.method === "GET") {
      ...
      const bulletins = await Promise.all(
        eleves.rows.map(async (eleve) => {
          const parMatiere: Record<
            string,
            { moyenne: number; coefficient: unknown; nbNotes: number }
          > = {};
          for (const matiere of matieres.rows) {
            const params: unknown[] = [eleve.id, matiere.id];
            let semestreFilter = "";
            if (semestre) {
              params.push(parseInt(semestre));
              semestreFilter = ` AND ev.semestre = $${params.length}`;
            }
            ...
            if (notes.rows.length > 0) {
              const moyenne =
                notes.rows.reduce((acc, n) => acc + parseFloat(String(n.valeur)), 0) /
                notes.rows.length;
          ...
          Object.values(parMatiere).forEach((m) => {
            moyenneGenerale += m.moyenne * parseFloat(String(m.coefficient));
            totalCoef += parseFloat(String(m.coefficient));
          });
      ...
      return json(cors, bulletins);
    }

    if (path === "/notes/bulletin-criteres" && req.method === "GET") {
      const classe_id = url.searchParams.get("classe_id");
      const semestre = url.searchParams.get("semestre");
      const sem = parseInt(semestre || "1") || 1;
      await pool.query(`
        DO $$ BEGIN
    ...
    const bulletinCriteresMatch = path.match(/^\/notes\/bulletin-criteres\/(\d+)$/);
    if (bulletinCriteresMatch && req.method === "PUT") {
      const eleve_id = bulletinCriteresMatch[1];
      const body = await parseJsonBody(req);
      const { classe_id, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, remarques, valide, semestre } =
        body;
      const sem = parseInt(String(semestre)) || 1;
      await pool.query(
        `
      ...
      return json(cors, { message: "Critères bulletin enregistrés" });
    }

    if (path === "/notes/rapport" && req.method === "GET") {
      const classe_id = url.searchParams.get("classe_id");
      const semestre = url.searchParams.get("semestre");
      ...
      const evalsParams: unknown[] = [classe_id];
      let evalsWhere = "WHERE ev.classe_id = $1";
      if (semestre) {
        evalsParams.push(parseInt(semestre));
        evalsWhere += ` AND ev.semestre = $${evalsParams.length}`;
      }
      ...
      for (const ev of evalsRes.rows) {
        if (!matiereMap[ev.matiere_id]) {
          matiereMap[ev.matiere_id] = {
            matiere_id: ev.matiere_id,
            matiere_nom: ev.matiere_nom,
            evaluations: [],
          };
        }
        ...
        matiereMap[ev.matiere_id].evaluations.push({
          ...
          date: ev.date,
          type: ev.type,
          coefficient: parseFloat(String(ev.coefficient)),
          prof_nom: ev.prof_nom,
          prof_prenom: ev.prof_prenom,
          notes: elevesRes.rows.map((e) => {
            const n = evalNotes.find((nn) => nn.eleve_id === e.id);
            return {
              eleve_id: e.id,
              valeur: n ? n.valeur : null,
              absent: n ? n.absent : null,
              dispense: n ? n.dispense : null,
            };
          }),
        });
      }

      return json(cors, { eleves: elevesRes.rows, matieres: Object.values(matiereMap) });
    }

    if (path === "/notes/suivi-classes" && req.method === "GET") {
      const result = await pool.query(`
    ...
    if (path === "/notes" && req.method === "GET") {
      ...
      const classe_id = url.searchParams.get("classe_id");
      const matiere_id = url.searchParams.get("matiere_id");
      const semestre = url.searchParams.get("semestre");
      let query = `
      ...
      `;
      const params: unknown[] = [classe_id];
      if (matiere_id) {
        query += ` AND ev.matiere_id = $${params.length + 1}`;
        params.push(matiere_id);
      }
      if (semestre) {
        query += ` AND ev.semestre = $${params.length + 1}`;
        params.push(parseInt(semestre));
      }
    ...
    if (path === "/notes" && req.method === "POST") {
      const body = await parseJsonBody(req);
      const { nom, classe_id, matiere_id, date, type, coefficient, sur, points_max, semestre, nb_exercices } =
        body;
      const result = await pool.query(
        ...
        [
          ...
          coefficient || 1,
          sur || 6,
          points_max != null && points_max !== "" ? points_max : null,
          semestre || 1,
          parseInt(String(nb_exercices)) || 0,
        ],
      );
      return json(cors, { message: "Evaluation creee", evaluation: result.rows[0] }, 201);
    }

    const notesEvalMatch = path.match(/^\/notes\/(\d+)\/notes$/);
    if (notesEvalMatch) {
      const eval_id = notesEvalMatch[1];
      ...
      if (req.method === "GET") {
        ...
        if (evaluation.rows.length === 0) return json(cors, { message: "Evaluation non trouvee" }, 404);
        ...
        return json(cors, { evaluation: evaluation.rows[0], eleves: elevesAvecNotes });
      }
      ...
      if (req.method === "POST") {
        ...
        try {
          ...
          for (const n of notes as Array<Record<string, unknown>>) {
            ...
            const det =
              n.points_detail && Object.keys(n.points_detail as object).length > 0
                ? JSON.stringify(n.points_detail)
                : null;
          ...
          return json(cors, { message: "Notes sauvegardees" });
        } catch (err) {
    ...
    if (idMatch) {
      ...
      if (req.method === "PUT") {
        const body = await parseJsonBody(req);
        const { nom, date, type, coefficient, points_max, nb_exercices } = body;
        const pm =
          points_max != null && points_max !== "" ? parseFloat(String(points_max)) : null;
        const client = await pool.connect();
        try {
          await client.query("BEGIN");
          await client.query(
          ...
          await client.query("COMMIT");
          return json(cors, { message: "Evaluation modifiee" });
        } catch (err) {
          await client.query("ROLLBACK");
          throw err;
        } finally {
      ...
      if (req.method === "DELETE") {
        ...
        return json(cors, { message: "Evaluation supprimee" });
      }
    }

    return json(cors, { message: "Route non trouvée" }, 404);
  } catch (err) {
    console.error("notes-fast error:", err);
