...
import type { Pool, PoolClient } from "npm:pg@8";
import { Buffer } from "node:buffer";
import { createPool } from "../auth-fast-shared.ts";
...
export function decodeDocument(contenu: unknown): Buffer {
  const s = String(contenu || "");
...
const fetchAnnee = async (client: Pool | PoolClient) => {
  try {
...
const sauvegarderArchiveAnneeDb = async (user: { id?: number } | null, pool: Pool) => {
...
export async function verifierArchivePourResetDb(archiveId: number, annee: string, pool: Pool) {
...
export async function verrouillerArchiveDb(archiveId: number, pool: Pool) {
  if (!archiveId) return;
  await pool.query("UPDATE archives_annees SET verrouilee=true WHERE id=$1", [archiveId]);
}

const listerArchivesDb = async (pool: Pool) => {
  const r = await pool.query(
...
const getArchiveDetailDb = async (id: string | number, pool: Pool) => {
...
const getArchiveFichierDb = async (archiveId: string | number, fichierId: string | number, pool: Pool) => {
  const r = await pool.query(
...
const exportArchiveZipBufferDb = async (archiveId: string | number, pool: Pool) => {
  const detail = await getArchiveDetailDb(archiveId, pool);
  ...
  const fichiers = await pool.query(
    `SELECT id, nom, eleve_id, contenu FROM archives_fichiers WHERE archive_id=$1`,
    [archiveId],
  );
  ...
  const byGroup = new Map<string, { title: string; folder: string; tables: unknown[] }>();
  tables.rows.forEach((t: Record<string, unknown>) => {
    const groupe = String(t.groupe);
    if (!byGroup.has(groupe)) {
      byGroup.set(groupe, {
        title: String(t.groupe_titre),
        folder: groupe,
        tables: [],
      });
    }
    byGroup.get(groupe)!.tables.push(t);
  });
  ...
  zip.append(
    await buildPdf(`Synthèse ${detail.annee_scolaire}`, [
      {
  ...
  for (const group of byGroup.values()) {
    const pdfSections: Array<{ heading: string; subtitle: string; rows: unknown[] }> = [];
    for (const t of group.tables as Array<Record<string, unknown>>) {
      const rows = Array.isArray(t.donnees) ? t.donnees : [];
      zip.append(sheetFromRows(rows, String(t.table_name)), {
        name: `${group.folder}/${t.table_name}.xlsx`,
      });
      pdfSections.push({
        heading: String(t.table_name),
        subtitle: `${rows.length} ligne(s)`,
        rows,
      });
    }
    zip.append(await buildPdf(group.title, pdfSections), {
      name: `${group.folder}/${group.folder}.pdf`,
    });
  }

  fichiers.rows.forEach((f: Record<string, unknown>, i: number) => {
...
export async function fetchAnneeMeta(pool?: Pool) {
  if (pool) return fetchAnnee(pool);
  return withPool((p) => fetchAnnee(p));
}

export async function sauvegarderArchiveAnnee(user: { id?: number } | null) {
  return withPool((pool) => sauvegarderArchiveAnneeDb(user, pool));
}

export async function verifierArchivePourReset(archiveId: number, annee: string) {
  return withPool((pool) => verifierArchivePourResetDb(archiveId, annee, pool));
}

export async function verrouillerArchive(archiveId: number) {
  return withPool((pool) => verrouillerArchiveDb(archiveId, pool));
}

export async function listerArchives() {
  return withPool((pool) => listerArchivesDb(pool));
}

export async function getArchiveDetail(id: string | number) {
  return withPool((pool) => getArchiveDetailDb(id, pool));
}
...
export async function getArchiveTable(
  ...
  return withPool((pool) => getArchiveTableDb(archiveId, tableName, pool, opts));
}

export async function getArchiveFichier(archiveId: string | number, fichierId: string | number) {
  return withPool((pool) => getArchiveFichierDb(archiveId, fichierId, pool));
}

export async function exportArchiveZipBuffer(archiveId: string | number) {
  return withPool((pool) => exportArchiveZipBufferDb(archiveId, pool));
}
