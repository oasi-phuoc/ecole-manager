...
export async function handleStatistiquesRoute(
  req: Request,
  path: string,
  cors: Record<string, string>,
): Promise<Response> {
  if (path !== "/statistiques" || req.method !== "GET") {
  ...
  try {
    ...
    if (!isAdmin) {
      ...
      classesAffectees = classesRes.rows.map((r) => r.classe_id as number);
    }

    const nbEleves = await safeQuery(
      pool,
      "SELECT COUNT(*) FROM eleves WHERE statut='actif'",
      [],
      [{ count: 0 }],
    );
    const nbProfs = await safeQuery(
      pool,
      "SELECT COUNT(*) FROM utilisateurs WHERE role='prof'",
      [],
      [{ count: 0 }],
    );
    const nbClasses = await safeQuery(pool, "SELECT COUNT(*) FROM classes", [], [{ count: 0 }]);
    const nbBranches = await safeQuery(pool, "SELECT COUNT(*) FROM matieres", [], [{ count: 0 }]);
    ...
    const classesDuJourRes = await safeQuery(
      ...
      ORDER BY c.nom
    `,
      isAdmin ? [jourAuj] : [jourAuj, user!.id],
      [],
    );

    let controlePresenceAujourdhui: Record<string, unknown> = {
      jour: jourAuj,
    ...
    if (creneauEnCoursRes.rows.length > 0) {
      ...
      const classesRes = await safeQuery(
        ...
        ORDER BY c.nom
      `,
        isAdmin ? [creneau.id] : [creneau.id, user!.id],
        [],
      );

      const classesEnCours: Record<string, unknown>[] = [];
      for (const cl of classesRes.rows) {
        ...
        const eleves = elevesRes.rows.map((el) => {
          const statut =
            el.p1 || el.p2 || el.p3 || el.p4 || el.p5 || el.p6 || el.p7 || el.p8 || "";
    ...
    let dernieresNotes: Record<string, unknown>[] = [];
    ...
    let dernieresObservations: Record<string, unknown>[] = [];
    if (isAdmin || classesAffectees.length > 0) {
      const params: unknown[] = [];
      let filtre = "";
      if (!isAdmin) {
    ...
    return json(cors, {
      nb_eleves: parseInt(String(nbEleves.rows[0].count)),
      nb_profs: parseInt(String(nbProfs.rows[0].count)),
      nb_classes: parseInt(String(nbClasses.rows[0].count)),
      nb_branches: parseInt(String(nbBranches.rows[0].count)),
    ...
    console.error("statistiques-fast error:", err);
